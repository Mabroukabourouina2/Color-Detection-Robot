    ...
    while True:
        ...
        if contours:
            for contour in contours:
                ...
                cv2.rectangle(res_frame, (x, y), (x + w, y + h), (255, 0, 0), 2)
                list.append([x, y, w, h])
                
            print(f"Detected {len(contours)} blue object(s).")
        ...
        for bbox in bbox_id:
            ...
            cx = int(x1 + w1 // 2)
            cy = int(y1 + h1 // 2)
            cv2.circle(res_frame, (cx, cy), 5, (255, 0, 0), -1) 
        ...
        cv2.imshow("Blue Detection", res_frame)
